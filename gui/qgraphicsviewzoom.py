...
class QGraphicsViewZoom(QGraphicsView):
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.is_panning:
            ...
            self.mouse_click_pos = event.pos()
            self.pan_offset += move_pos / self.scale_factor
        # Preserve functionality of parent's implementation
    ...
    def getVisibleBounds(self):
        ...
        # Calculate the bounds based on scale factor and pan offset
        top_left = self.pan_offset
        bottom_right = self.pan_offset + QPointF(width, height) / self.scale_factor
